...
class EslembicTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.original_check_migration_possible = (
            chartreuse.utils.EslembicMigrationHelper.check_migration_possible
        )
    ...
    def test_clean_index(self):
        ...
        with mock.patch("os.chdir"):
            with mock.patch(
                ...
                self.eslembicMigrationHelper = chartreuse.utils.EslembicMigrationHelper(
                    "foo", True
                )
                self.eslembicMigrationHelper.migrate_db()
                mocked_run_command.assert_called_with(
                    "eslembic upgrade head --clean-index"
                )
