...
def test_postdeployment_detected_migration_enabled_stop_pods(mocker):
    """
    Test that postdeployments stop pods in case of detected migration.
    """
    configure_chartreuse_mock(mocker=mocker, is_migration_needed=True)
    mocked_stop_pods = mocker.patch("chartreuse.postdeployment.stop_pods")
...
def test_postdeployment_detected_migration_disabled_stop_pods(mocker):
    """
    Test that postdeployments does not stop pods in case of detected migration but we disallow stop-pods.
    """
    configure_chartreuse_mock(mocker=mocker, is_migration_needed=True)
    mocked_stop_pods = mocker.patch("chartreuse.postdeployment.stop_pods")
...
def test_postdeployment_no_migration_disabled_stop_pods(mocker):
    """
